document.addEventListener('DOMContentLoaded', function() {
    ...
    function run() {
        ...
        const moleIndexes = []; // Array to hold the indexes of the mole images to be shown
        while (moleIndexes.length < numMoles) {
            const randomIndex = Math.floor(Math.random() * moleImages.length);
            if (!moleIndexes.includes(randomIndex)) {
                moleIndexes.push(randomIndex);
            }
        }

        moleIndexes.forEach(moleIndex => {
            const holeIndex = getRandomInterval(0, holes.length - 1);
            const hole = holes[holeIndex];

            const img = document.createElement('img');
            img.classList.add('mole');

            // Use custom mole image if available, otherwise use default
            const moleImage = customMoleImages[moleIndex] || moleImages[moleIndex];
            img.src = moleImage;
            ...
            img.addEventListener('click', (event) => {
                ...
                if (distance <= holeRect.width / 2) {
                    ...
                    const voice = customMoleVoices[moleIndex] || "smash.mp3"; // Use custom voice if available, otherwise default
                    playAudio(voice);
                }
            });
